function [sigma0vvdB sigma0hhdB sigma0hvdB] = FdBLangSidhu(f,thetai,lr,ur)
%% 
% Frequency (GHz)
%f = 1.8;
...
% Oriented volume cases in theta direction ; not ranging between 0 to pi
lr = deg2rad(lr); % lower range of theta
ur = deg2rad(ur); % close to 10 deg for parallel disk (to ground) and close to 60 deg for verical
rr = abs(ur-lr);
% calculation axy
f1 = @(th,ph) (1/rr).*(1/(2*pi)).*(((ar.*(sin(th).^2) + at.*(cos(th).^2)) - ap).*cos(ph).*sin(ph));
axy = integral2(f1,lr,ur,0,2*pi);
% calculation axz
f2 = @(th,ph) (1/rr).*(1/(2*pi)).*((ar-at).*sin(th).*cos(th).*cos(ph));
axz = integral2(f2,lr,ur,0,2*pi);
% calculation ayz
f3 = @(th,ph) (1/rr).*(1/(2*pi)).*((ar-at).*sin(th).*cos(th).*sin(ph));
ayz = integral2(f3,lr,ur,0,2*pi);
% calculation axx and ayy, azz
f4 = @(th,ph) (1/rr).*(1/(2*pi)).*(((ar.*(sin(th).^2) + at.*(cos(th).^2)).*(cos(ph).^2)) + (ap.*(sin(ph).^2)));
axx = integral2(f4,lr,ur,0,2*pi);
f5 = @(th,ph) (1/rr).*(1/(2*pi)).*(((ar.*(sin(th).^2) + at.*(cos(th).^2)).*(sin(ph).^2)) + (ap.*(cos(ph).^2)));
ayy = integral2(f5,lr,ur,0,2*pi);
f6 = @(th,ph) (1/rr).*(1/(2*pi)).*(ar.*(cos(th).^2) + at.*(sin(th).^2));
azz = integral2(f6,lr,ur,0,2*pi);
