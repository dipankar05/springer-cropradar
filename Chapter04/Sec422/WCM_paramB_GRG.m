...
loglog(mg,abs(Bgrgvv(:,kk)'),'o','MarkerEdgeColor','None','MarkerFaceColor',colors_p(kk,:))
hold on
grid on
...
figure('DefaultAxesFontSize',14)
loglog(vwc(:),abs(Bgrgvv(:)),'o','MarkerEdgeColor','None','MarkerFaceColor',[0 0 1])
hold on
grid on
...
% RGB Table help: https://www.rapidtables.com/web/color/RGB_Color.html
colors_p = [linspace(blue(1),cyan(1),length)', linspace(blue(2),cyan(2),length)', linspace(blue(3),cyan(3),length)'];
loglog(mg,abs(Bgrghh(:,kk)'),'o','MarkerEdgeColor','None','MarkerFaceColor',colors_p(kk,:))
hold on
grid on
xlabel('Particle moisture content $m_g (g~g^{-1})$','Interpreter','latex');
